...
#------------------------------------------------------------------
PROJNAME     := hello-simon
VENDOR       := gavinmcnair
MAINTAINER   := gavinmcnair@gmail.com
...
#------------------------------------------------------------------
DOCKER_REPO	   		:= gavinmcnair
DOCKER_USERNAME  		:= gavinmcnair+hello_simon
DOCKER_PASSWORD  		?= "unknown"
CIRCLE_BUILD_NUM	?= "unknown"
IMG_VERSION			:= 1.1.$(CIRCLE_BUILD_NUM)
IMAGE 				:= docker.io/$(DOCKER_REPO)/$(PROJNAME):$(IMG_VERSION)
...
.PHONY: all
all: test hello-simon  ## Run test and hello-simon  (default)
...
docker-push: ## Pushes to docker images to docker.io
	docker push $(IMAGE)
	docker rmi $(IMAGE)
